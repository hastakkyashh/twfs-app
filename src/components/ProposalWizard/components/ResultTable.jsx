...
const ResultTable = ({
  ...
  const handleEquityChange = (strategyId, value) => {
    ...
    setTempAllocations((prev) => ({
      ...prev,
      [strategyId]: { equity: numValue, debt: 100 - numValue },
    }));
  };
  ...
  const handleDebtChange = (strategyId, value) => {
    ...
    setTempAllocations((prev) => ({
      ...prev,
      [strategyId]: { equity: 100 - numValue, debt: numValue },
    }));
  };
    ...
    <div>
      <div className="flex justify-end mb-4 gap-3">
        {!editMode ? (
          ...
          >
            <Edit2 className="w-4 h-4" /> Edit Allocations
          </button>
        ) : (
          <>
            <button
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse bg-white shadow-lg rounded-lg overflow-hidden">
                    ...
                    <div className="flex gap-2 items-center justify-center">
                      <div className="flex flex-col">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleEquityChange(strategy.id, e.target.value)
                          }
                      ...
                      <div className="flex flex-col">
                        ...
                        <input
                          type="number"
                          value={
                            tempAllocations[strategy.id]?.debt ?? strategy.debt
                          }
                          onChange={(e) =>
                            handleDebtChange(strategy.id, e.target.value)
                          }
                          className="w-16 px-2 py-1 text-sm text-gray-900 border border-gray-300 rounded"
                        />
                      </div>
          ...
          <tbody>
            ...
            {projections[0]?.projections.map((p, idx) => (
              <tr
                key={p.year}
                className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}
              >
                <td className="border border-gray-300 p-4">
                  <div className="font-bold text-gray-900">Year {p.year}</div>
                  <div className="text-xs text-gray-500">
                    Invested: {formatCurrency(p.totalInvestment)}
                  </div>
                </td>
                {projections.map((strategy) => {
                  const data = strategy.projections[idx];
                  return (
                    <td
                      key={strategy.id}
                    ...
                    >
                      <div className="font-bold text-lg text-gray-900">
                        {formatCurrency(
                          strategy.projections[idx].probableAmount,
                        )}
                      </div>
                      <div className="text-sm text-green-600 mt-1">
                        ...
                        {formatNumber(
                          ((strategy.projections[idx].probableAmount -
                            strategy.projections[idx].totalInvestment) /
                            strategy.projections[idx].totalInvestment) *
                            100,
                        )}
                        % gain
                      ...
                      <div className="text-[10px] text-gray-400 uppercase">
                        Growth Rate: {strategy.return}% p.a.
                      </div>
                    </td>
                  );
                })}
              ...
              {projections.map((strategy) => (
                ...
                >
                  <button
                    ...
                    className={`inline-flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all ${selectedStrategies === strategy.id ? "bg-blue-600 border-blue-600 text-white" : "bg-white border-gray-300 hover:border-blue-400"}`}
                  >
